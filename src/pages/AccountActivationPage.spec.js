...
describe("Account Activation Page", () => {
  ...
  it("displays account activation success message when token is correct", async () => {
    render(AccountActivationPage, { token: "1234" });
    const successMessage = await screen.findByText("Account is activated");

    expect(successMessage).toBeInTheDocument();
  });
  it("sends activation token to backend", async () => {
    render(AccountActivationPage, { token: "1234" });
    await screen.findByText("Account is activated");

    expect(counter).toBe(1);
  });
  it("displays account activation failure message when token is incorrect", async () => {
