...
describe("Routing", () => {
  ...
  it.each`
    ...
    ${"/signup"} | ${"Sign Up"}
    ${"/login"} | ${"Login"}
  `("has link to homepage on NavBar", ({ path, queryName }) => {
    setup("/");
  ...
  `("displays $visible after clicking $clicking link", async ({ initialPath, clicking, visible, lastUrl }) => {
    setup(initialPath);
  ...
  it("displays home page when clicking on brand logo", async () => {
    setup("/login");
    const img = screen.queryByAltText("Hoaxify");
